...
// 5/4/2022
// Project 3
// Description: added cache to program  
//
...
struct cacheentry Thecache[m];

int num_hits = 0;
int num_misses = 0;


unsigned char cacheEnter(){

    printf("AR being sought from the cache = %d \n", AR);
    unsigned short tempAR = AR;
    unsigned short temp = AR;
    temp = temp >> 2;
    //push of the last 2 bits 
    //temp = temp & 0xFFFC;//would clear the last two bits but still need to be moved so why do at all??
    //t5 = t5 & 0xFFFC;

    printf("The main memory block number = %d \n", temp);
    unsigned char temp2 = 0;//temp && 0x03FC;//would get just the 8 need on but the shift then change to char does the same
    temp2 = temp;
    //clears the upper bits still on by changing to char
    
    printf("The cacheline associated with AR = %d \n", temp2);
    //Thecache[temp2].line[0] = Thecache[temp2].line[0] + 3;  
    printf("Entry %d tag = %d data = %d %d %d %d \n", temp2, Thecache[temp2].tag, Thecache[temp2].line[0], Thecache[temp2].line[1], Thecache[temp2].line[2], Thecache[temp2].line[3]);
    
    if(Thecache[temp2].tag == invalid_tag){
        //cache not hit update data
        printf("Cache miss \n");
        Thecache[temp2].tag = temp >> 8;
        num_misses++;

        Thecache[temp2].line[0] = M[AR + 0 - (tempAR & 3)];
        Thecache[temp2].line[1] = M[AR + 1 - (tempAR & 3)];
        Thecache[temp2].line[2] = M[AR + 2 - (tempAR & 3)];
        Thecache[temp2].line[3] = M[AR + 3 - (tempAR & 3)];
        /*
        Thecache[temp2].line[0] = M[AR];
        Thecache[temp2].line[1] = M[AR + 1];
        Thecache[temp2].line[2] = M[AR + 2];
        Thecache[temp2].line[3] = M[AR + 3];
        */
    ...
    //return the data.
    return Thecache[temp2].line[tempAR%4];
...
unsigned char cacheupdate(){

    printf("Cache about to be updated due to a write-through, cache before\n");
    ...
    printf("The main memory block number = %d \n", temp);
    unsigned char temp2 = 0;//temp && 0x03FC;
    temp2 = temp;
    ...
    if(Thecache[temp2].tag == invalid_tag){
        ...
        //updated either way so not necessary to have a conditional if
        Thecache[temp2].line[0] = M[AR + 0 - (tempAR & 3)];
        Thecache[temp2].line[1] = M[AR + 1 - (tempAR & 3)];
        Thecache[temp2].line[2] = M[AR + 2 - (tempAR & 3)];
        Thecache[temp2].line[3] = M[AR + 3 - (tempAR & 3)];
        ...
        Thecache[temp2].line[tempAR%4] = DR;
...
void STAC(){
    ...
    //cacheupdateSTAC();
    unsigned char dataupdate = cacheupdate();
...
void ADD16(){
    ...
    //check for carry
    if(temp2 >= 65536 || temp1+tempR <-32767){
        Flag = Flag + C;
    }
    ...
    //if ac and 1 are pos and ans is neg overflow flag should be set
    //if ac and 1 are neg and ans is pos then flag is set
    if(temp1 <= 32767 && tempR <= 32767 && temp2 < 0){      
        Flag = Flag + V;
    }
    if(temp1 < 0 && tempR < 0 && temp2 <= 32767){
        Flag = Flag + V;
    }
    
    //check for negative
    if(temp2 > 32767 || temp2 < 0){
        Flag = Flag + N;
    }
...
int main(){
    ...
    cout<<"Project 3: rscpuecache"<<endl;
    cout<<"Cache added.\n"<<endl;
    //
    ...
    for(int i = 0; i < m; i++){
        Thecache[i].tag = invalid_tag;
        //printf(" me please is thi the data  = %d  \n", Thecache[i].tag);
        for(int j = 0; j < 4; j++){
            Thecache[i].line[j] = 0;
            //printf(" me please is thi the data  = %d  \n", Thecache[i].line[j]);
        }
